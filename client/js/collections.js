...
function displayCollectionsMenu() {	
	...
	if (sharedWithMe.length > 0) {
		$(".collections-menu").append('<li><a class="subheader">Delade med mig</a></li>');
		for(var i=0; i<sharedWithMe.length; i++){
			$(".collections-menu").append('<li><a class="image-collection shared-collection waves-effect dnd-target" href="#" data-id="' + sharedWithMe[i].id + '">' + sharedWithMe[i].name + "</a></li>");
		}
		$(".collections-menu").append('<li><div class="divider"></div></li>');
	}
	
	if (inboxImages.length > 0) {
		$(".collections-menu").append('<li><a class="subheader">Inbox</a></li>');
		for(var i=0; i<inboxImages.length; i++){
			$(".collections-menu").append('<li><a class="inbox-image waves-effect" href="#" data-id="' + inboxImages[i].id + '"><i class="material-icons">perm_media</i>' + inboxImages[i].name + "</a></li>");
		}
		$(".collections-menu").append('<li><div class="divider"></div></li>');
	}
